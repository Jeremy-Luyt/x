...
- 在“新增”的专用发票页面，点击“检测 U8”后，程序会重新验证页面和供应商弹窗状态。逐项确认后仅可填写开票日期、供应商发票号、税率、币种。
- 已验证的“采购供应商档案”窗口可使用“供应商参照：按本任务名称筛选”：程序只会在检索框填写 PDF 中可靠提取的供应商名称并点击“过滤”。用户必须核对结果、手工选中记录并在 U8 内点击“确定”；程序不选择供应商、不点击确定。
...
`src/u8/purchase_invoice_workflow.py` 提供供应商规范化比对、单行 `Decimal` 金额/税额/价税合计校验、结构化失败结果和 Grid capability detection。真实诊断已确认供应商参照是无标题 `ThunderRT6FormDC` 窗体，且其“供应商简称 / 包含 / 检索框 / 过滤 / VSFlexGrid8U 结果表格”可唯一识别，因此只启用了安全的筛选步骤。结果表格和发票明细控件仍是自绘 Grid，尚未采到可读取的行文本或单元格；程序不会猜测、按坐标点击行或自动确认供应商。

要继续该 workflow 的实机验证，请分别采集以下状态的诊断 ZIP：

1. 新增专用采购发票空白页；
2. 供应商参照弹窗筛选后、仅剩一个结果但尚未点击“确定”；
3. 存货参照弹窗刚打开；
4. 第一行数量单元格进入编辑状态；
5. 第一行原币单价单元格进入编辑状态。

只有拿到结果行可读取的名称、唯一性证据及回读结果后，才会考虑供应商自动选择；只有拿到单元格可读取/可回读证据后，才会考虑单行 Grid 的键盘策略。任何 selector 不唯一、回读不一致或意外弹窗都会停止，交由人工处理。
